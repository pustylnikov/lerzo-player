...
- `PlayerSurfaceView.swift` — NSView под окно mpv; mpv рисует в собственном NSWindow,
  который всегда прикреплён к окну приложения как child window (в полноэкранном
  режиме — на уровне `normal-1`, чтобы mpv не вывел его поверх оверлея). Отсоединять
  его в полноэкранном режиме нельзя: отдельное окно не попадает в анимацию переключения
  Spaces, и при возврате на Space с видео был чёрный кадр с последующим fade-in.
  ...
  Поведение окна mpv: `[.fullScreenAuxiliary]`, никогда `.canJoinAllSpaces` (видео
  появлялось на других рабочих столах).
